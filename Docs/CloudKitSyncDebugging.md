# Mythos Log CloudKit Sync Debugging

Mythos Log's production SwiftData store should be created by `TrainingStore.makeModelContainer()` with:

```swift
cloudKitDatabase: .private("iCloud.studio.curateddesign.MythosLog")
```

In Debug, the shared `MythosLog` scheme enables:
...
Run both the physical iPhone and the Simulator from Xcode with the `MythosLog` scheme. Filter the console for `SwiftDataCloudKit`, `CloudKit`, or `CoreData`.

What to check:

- `ModelContainer created` should say `cloudKit=private(iCloud.studio.curateddesign.MythosLog)`.
...
3. Check Settings > Sync Diagnostics. The second device should eventually show `Container` as `iCloud.studio.curateddesign.MythosLog` and record counts should rise without reseeding local defaults first.
